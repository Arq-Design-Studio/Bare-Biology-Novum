...
ReCharge.Novum.Components = {
  ...
  renderExpandableCard(element) {
    ...
    iconContainer.classList.add('px-2', 'rc-expandable-card--arrow');
...
ReCharge.Novum.DomCreators = {
  ...
  getAssociatedSubscriptionsDom(subscriptions = []) {
    ...
    return `
      <div class="associated-subscriptions text-body-2">
        <h4 class="rc-subheading mt-4">${translations.associatedSubscriptions.header}</h4>
        ${subscriptions.length === 0 ? `<div>${translations.associatedSubscriptions.empty}</div>` : ''}
        <div class="subscription">
          ${subscriptions.map(subscription => `
            <div class="subscription-item">${subscription.product_title} - ${ReCharge.Utils.formatIntervalFrequency(subscription.order_interval_frequency, subscription.order_interval_unit)}</div>
          `).join('\n')}
        </div>
      </div>`
  },
  ...
  getPaymentMethodDetailsDom(paymentMethod) {
    const { translations } = ReCharge;
    const { capitalize } = ReCharge.Utils;
    ...
    const brand = ReCharge.Utils.formatReadable(paymentDetails && paymentDetails.brand);
    const isVaulted = ReCharge.Utils.isVaulted(paymentMethod);
    const cardDetails =  brand && paymentDetails
      ? translations.paymentMethod.last4.replace('{brand}', capitalize(brand)).replace('{last4}', paymentDetails.last4 ?? '')
      : ReCharge.Utils.formatReadable(paymentType, { capitalize: true })
    
    return `
      <div class="payment-method-details flex-1 text-body-2" data-id="${paymentMethod.id}">
        <div class="payment-type">
        ${isVaulted ? translations.subscription.vaultedPaymentMethod : cardDetails}
        </div>
        ${paymentDetails ? render(paymentDetails.exp_month && paymentDetails.exp_year && `
          <div class="expires">${translations.paymentMethod.expires} ${paymentDetails.exp_month}/${paymentDetails.exp_year}</div>`) : render(`<div class="expires"></div>`)}
      </div>`;
  },
